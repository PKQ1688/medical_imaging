...
def get_use_data():
    mask_data_list = os.listdir("data/res_data/mask_img/")
    for mask_name in mask_data_list:
        ...
        mask_img_list_small = sorted(
            os.listdir(f"data/res_data/mask_img/{mask_name}"), key=lambda x: int(x.split(".")[0]))
        img_list_small = sorted(os.listdir(f"data/res_data/origin_img/{mask_name}"),
        ...
        for img_name, mask_img_name in zip(img_list_small, mask_img_list_small):
            # print(img_name)
            # print(mask_img_name)
            img = cv2.imread(f"data/res_data/origin_img/{mask_name}/{img_name}")
            mask_img = cv2.imread(f"data/res_data/mask_img/{mask_name}/{mask_img_name}")
...
def adjust_image_orientation():
    mask_img_list = sorted(os.listdir("data/res_data/mask/"),
    ...
    for mask_img_name in mask_img_list:
        ...
        #
        # # 保存或返回翻转后的图像
        cv2.imwrite(f"data/res_data/mask_flip/{mask_img_name}", img_transposed)
        # # 或者直接显示
...
def get_coco_annotation(is_train=True):
    ...
    for img_id, (image_path, mask_path) in enumerate(zip(image_paths, mask_paths), 1):
        ...
        category_name = int(label_df[label_df["number"] == file_index]["label"].values[0])
        # 添加标注信息
...
def validate_annotation():
    ...
    annFile = 'data/res_data/ct_dataset.json'
    image_id = 1  # 标注文件路径
    coco = COCO(annFile)
